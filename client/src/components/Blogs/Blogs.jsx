  ...
  return (
    <div className="flex p-2 justify-between rounded-lg">
      <div className="w-[70%]">
        ...
        <span className="mt-4 font-thin">{new Date(post.createdAt).toDateString().substring(3)}</span>
        <Link
          to={`/post/${post._id}`}
          onClick={() => {
            window.scroll({
      ...
      <div className="w-[30%]">
        {post.photo ? (
          <img
            className="postImg mt-10 object-cover object-center w-[300px] h-[100px]  ml-10 mb-8 lg:h-48 md:h-36 rounded-xl"
            src={pf + post.photo}
            alt=""
          />
        ) : (
          <img
            className="postImg mt-10 object-cover ml-10 object-center w-[300px] h-[100px] mb-8 lg:h-48 md:h-36 rounded-xl"
